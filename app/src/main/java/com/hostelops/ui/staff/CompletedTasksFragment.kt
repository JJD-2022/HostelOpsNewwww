...
class CompletedTasksFragment : Fragment() {
    ...
    private fun loadComplaints() {
        db.collection("complaints")
            .whereIn("status", listOf("RESOLVED", "COMPLETED"))
    ...
    private fun filterAndDisplay(query: String) {
        ...
        if (filtered.isEmpty()) {
            ...
            binding.rvComplaints.adapter = ComplaintAdapter(filtered) { complaint ->
                val action = CompletedTasksFragmentDirections.actionCompletedTasksFragmentToComplaintDetailFragment(complaint.id)
